...
/**
 * Three tap-to-pick fields (Day/Month/Year) instead of a free-typed
 * "YYYY-MM-DD" box — same full-screen sheet pattern as CountyPicker/
 * CampusPicker. Still reports a single "YYYY-MM-DD" string via onChange
 * (only once all three are picked), so callers built around that format
 * (validation regex, the PATCH /auth/me body) need no changes.
 */
export default function DateOfBirthPicker({ value, onChange }) {
  ...
  // Descending — most recently eligible birth year first, oldest last.
  const years = useMemo(
