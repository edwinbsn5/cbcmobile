...
export default function MyBoostedGroupsScreen() {
  ...
  async function handleResume(id) {
    try {
      await client.post(`/group-boosts/${id}/resume`);
      load();
    } catch (e) {
  ...
  return (
    <FlatList
      ...
      keyExtractor={(b) => b.id}
      ListEmptyComponent={
        <Text style={styles.empty}>You haven't boosted any groups yet. Boost a group you admin from its page to feature it in the home feed.</Text>
      }
      renderItem={({ item }) => {
        ...
        return (
          <View style={styles.card}>
            <Text style={styles.content} numberOfLines={2}>{item.group?.name || "(group deleted)"}</Text>
