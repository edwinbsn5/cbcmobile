...
export default function MySponsoredPostsScreen() {
  ...
  const load = useCallback(() => {
    client.get("/boosts/mine").then((r) => setBoosts(r.data)).finally(() => setLoading(false));
  }, []);
  ...
  function handleDelete(id) {
    Alert.alert("Delete boost", "Stop boosting this post? This cannot be undone.", [
      ...
      { text: "Delete", style: "destructive", onPress: async () => { await client.post(`/boosts/${id}/delete`); load(); } },
    ]);
  }
