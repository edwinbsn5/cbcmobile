...
export default function ReportPostScreen({ route, navigation }) {
  const { postId, groupId, pageId, eventId, submissionId, collabGroupKind, collabGroupId } = route.params;
  ...
  async function handleSubmit() {
    ...
    try {
      const path = submissionId
        ...
        : eventId
        ? `/events/${eventId}/posts/${postId}/report`
        : collabGroupKind
        ? `/${collabGroupKind === "chama" ? "chama" : "projects"}/${collabGroupId}/posts/${postId}/report`
        : `/feed/${postId}/report`;
