...
// Reports the sponsored PLACEMENT itself (wrong targeting, scam, irrelevant
// ad, etc.) — mirrors ReportPostScreen exactly, just posting to
// /ad-reports instead. A boosted post's own CONTENT is still reportable
// separately via its normal PostCard "..." menu (AdCard renders the real
// PostCard for those), so this is deliberately about the ad, not the post.
export default function ReportAdScreen({ route, navigation }) {
  const { adType, adRefId, advertiserName } = route.params;
  const [reason, setReason] = useState(null);
  const [details, setDetails] = useState("");
  const [submitting, setSubmitting] = useState(false);

  async function handleSubmit() {
    if (!reason) return Alert.alert("Pick a reason", "Select why you're reporting this ad");
    setSubmitting(true);
    try {
      await client.post("/ad-reports", { adType, adRefId, reason, details: details.trim() || undefined });
      Alert.alert("Report submitted", "Thanks — our team will review this ad.");
      navigation.goBack();
    } catch (e) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      <Text style={styles.title}>Why are you reporting this ad{advertiserName ? ` from ${advertiserName}` : ""}?</Text>
