...
export default function MarketBoostProductScreen({ route, navigation }) {
  ...
  // decides what to show here. Tiers themselves come from the backend
  // (services/boostTiers.js) rather than being hardcoded — one source of
  // truth for pricing.
  useEffect(() => {
  ...
  async function handleBoost() {
    ...
    try {
      ...
      Alert.alert(
        "Listing boosted!",
        data.usedFreeBoost
          ? `Your listing may now appear as a featured pick for the next ${durationLabel} — this month's free Gold tier perk.`
          : `Your listing may now appear as a featured pick in Browse by Photos/Videos for the next ${durationLabel}.`
      );
      navigation.goBack();
    } catch (e) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 16 }}>
      ...
      {freeBoostAvailable && (
        <Text style={styles.hint}>🔵 Free this month — your Gold tier benefit covers whichever plan you pick below</Text>
      )}
