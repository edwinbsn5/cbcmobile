...
export default function MyBoostedEventsScreen() {
  const [boosts, setBoosts] = useState([]);
  const [loading, setLoading] = useState(true);

  const load = useCallback(() => {
    client.get("/event-boosts/mine").then((r) => setBoosts(r.data))
  ...
  async function handlePause(id) {
    try {
      await client.post(`/event-boosts/${id}/pause`);
      load();
    } catch (e) {
  ...
  async function handleResume(id) {
    try {
      await client.post(`/event-boosts/${id}/resume`);
      load();
    } catch (e) {
  ...
  function handleDelete(id) {
    Alert.alert("Delete boost", "Stop boosting this event? This cannot be undone.", [
      { text: "Cancel", style: "cancel" },
      { text: "Delete", style: "destructive", onPress: async () => {
          try {
            await client.post(`/event-boosts/${id}/delete`);
            load();
          } catch (e) {
  ...
  return (
    <FlatList
      ...
      keyExtractor={(b) => b.id}
      ListEmptyComponent={
        <Text style={styles.empty}>You haven't boosted any events yet. Boost an event you're hosting from its event page to pin it to the top of the Events list.</Text>
      }
      renderItem={({ item }) => {
        ...
        return (
          <View style={styles.card}>
            <Text style={styles.content} numberOfLines={2}>{item.event?.name || "(Event deleted)"}</Text>
